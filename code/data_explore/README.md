...
2.  **資料準備：** 確保輸入資料（JSON 或 CSV）可被讀取。系統目前預設讀取 `test_file/test.json`，或可於 `utilities/get_patient_info.py` 中設定特定的 CSV 路徑。
...
2.  **資料讀取 (`get_patient_info.py`)：** 從 JSON 或 CSV 讀取病患資訊（生命徵象、主訴等）並進行格式化。
...
-   本專案已全面導入 `logging` 機制，執行過程中的資訊（包含時間、狀態、錯誤訊息）皆會輸出至 Console。
